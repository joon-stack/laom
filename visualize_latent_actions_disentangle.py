...
from src.nn import DisentangledLAOM
...
def visualize(args):
    """
    지정된 궤적의 뷰별 잠재 행동을 UMAP으로 시각화합니다 (DisentangledLAOM 모델용).
    - 색상: 실제 행동 그룹 (Action Bin)
    - 모양: 뷰 (View)
    - z_a (action representation) 사용
    """
    print("--- 1. 설정 파일 로딩 ---")
    ...
    model_kwargs = {
        'shape': (3 * lapo_config['frame_stack'], dataset.img_hw, dataset.img_hw),
        'latent_act_dim': lapo_config['latent_action_dim'],
        'latent_view_dim': lapo_config['latent_action_dim'],  # Same size as latent_act_dim
    ...
    load_checkpoint(lapo, args.checkpoint_path)
    lapo.eval()
    ...
    for traj_idx, t, view_key in tqdm(sampled_combinations, desc="Extracting latent actions"):
        ...
        with torch.autocast(DEVICE, dtype=torch.bfloat16):
            z_a, z_v, obs_pred, obs1_emb = lapo(obs, future_obs)
        
        # Use action representation (z_a) for visualization
        latent_actions.append(z_a.squeeze().cpu().float().numpy())
        # Also collect view representation (z_v)
        latent_views.append(z_v.squeeze().cpu().float().numpy())
    ...
    print("\n--- 5. 실제 행동 군집화 ---")
    latent_actions_array = np.array(latent_actions)
    ...
    # z_v embedding
    latent_views_array = np.array(latent_views)
    reducer_v = umap.UMAP(n_components=2, random_state=42)
    embedding_v = reducer_v.fit_transform(latent_views_array)
    ...
    action_markers = {bin_name: markers_list[i % len(markers_list)] for i, bin_name in enumerate(unique_bins)}

    # 1. 모든 뷰와 액션 bin을 함께 시각화 (색상=액션, 모양=뷰)
    print("Generating combined plot for all views (color=view)...")
    ...
    plt.title(f'DisentangledLAOM UMAP: Color=View (z_a) ({len(latent_actions)} samples from {num_traj_to_process} trajectories)')
    ...
    all_views_path = os.path.join(output_dir, f"{base_filename}_za_all_views_combined.png")
    plt.savefig(all_views_path)
    print(f"전체 통합 시각화 결과를 '{all_views_path}'에 저장했습니다.")
    plt.close()

    # 2. 각 뷰를 개별적으로 시각화 (모양=action_bin, 색상=action_bin)
    for view_to_highlight in tqdm(unique_views, desc="Generating plots for each view"):
        ...
        plt.title(f'DisentangledLAOM UMAP for {view_to_highlight} (z_a) ({len(latent_actions)} samples from {num_traj_to_process} trajectories)')
        plt.xlabel('UMAP Dimension 1')
        plt.ylabel('UMAP Dimension 2')
        plt.grid(True)
        plt.tight_layout()
        
        view_path = os.path.join(output_dir, f"{base_filename}_za_{view_to_highlight.replace('/', '_')}.png")
        plt.savefig(view_path)
        plt.close()
    
    print(f"각 뷰별 개별 시각화 결과를 '{output_dir}/{base_filename}_*.png' 패턴으로 저장했습니다.")

    # 3. z_v 기반 통합 시각화 (색상=액션 bin, 모양=뷰)
    print("Generating combined plot for all views (z_v, color=view)...")
    plt.figure(figsize=(16, 14))
    sns.scatterplot(
        data=df_v,
        x='umap1',
    ...
    plt.title(f"DisentangledLAOM UMAP: Color=View (z_v) ({len(latent_views)} samples from {num_traj_to_process} trajectories)")
    plt.xlabel('UMAP Dimension 1')
    plt.ylabel('UMAP Dimension 2')
    plt.legend(title='Legend', bbox_to_anchor=(1.05, 1), loc='upper left', ncol=1)
    plt.grid(True)
    ...
    all_views_path_v = os.path.join(output_dir, f"{base_filename}_zv_all_views_combined.png")
    plt.savefig(all_views_path_v)
    print(f"z_v 전체 통합 시각화 결과를 '{all_views_path_v}'에 저장했습니다.")
    plt.close()

    # 4. z_v 각 뷰별 시각화 (모양=action_bin, 색상=action_bin)
    for view_to_highlight in tqdm(unique_views, desc="Generating plots for each view (z_v)"):
        plt.figure(figsize=(12, 10))
        
        highlight_df_v = df_v[df_v['view'] == view_to_highlight]
        sns.scatterplot(
            data=highlight_df_v, 
            x='umap1', 
            y='umap2',
        ...
        plt.title(f"DisentangledLAOM UMAP for {view_to_highlight} (z_v) ({len(latent_views)} samples from {num_traj_to_process} trajectories)")
        plt.xlabel('UMAP Dimension 1')
        plt.ylabel('UMAP Dimension 2')
        plt.grid(True)
        plt.tight_layout()
        
        view_path_v = os.path.join(output_dir, f"{base_filename}_zv_{view_to_highlight.replace('/', '_')}.png")
        plt.savefig(view_path_v)
        plt.close()


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Visualize latent actions from a DisentangledLAOM model using UMAP, with color for action bin and shape for view.")
    
    parser.add_argument('--checkpoint-path', type=str, required=True, 
                        help='Path to the DisentangledLAOM model checkpoint (.pt file)')
    ...
    parser.add_argument('--num-visualize', type=int, default=50, 
                        help='Number of samples to visualize after loading all trajectories (default: 50)')
