...
class Config:
    ...
    only_decoder: OnlyDecoderConfig = field(default_factory=OnlyDecoderConfig)
...
def train_only_decoder(config: OnlyDecoderConfig, checkpoint_dir: str):
    dataset = DCSDecoderInMemoryDataset(config.data_path, frame_stack=config.frame_stack, offset=config.offset, device="cpu")
    dataloader = DataLoader(
        dataset,
        batch_size=config.batch_size,
        shuffle=True,
    ...
    for epoch in trange(config.num_epochs, desc="Epochs"):
        ...
        for batch in tqdm(dataloader, desc=f"Epoch {epoch+1}/{config.num_epochs}", leave=False):
            ...
            # print("INFO: true_actions.shape: ", true_actions.shape)
            # print("INFO: true_actions: ", true_actions)
            # rescale from 0..255 -> -1..1
            obs = normalize_img(obs.permute((0, 3, 1, 2)))
            ...
            wandb.log(
                {
                    "only_decoder/mse_loss": loss.item(),
                    "only_decoder/throughput": total_tokens / (time.time() - start_time),
                    "only_decoder/learning_rate": scheduler.get_last_lr()[0],
                    "only_decoder/epoch": epoch,
                    "only_decoder/total_steps": total_steps,
                }
            )
    ...
    save_checkpoint(
        ...
        config.num_epochs - 1,
        loss.item(),
        os.path.join(checkpoint_dir, "only_decoder_final.pt"),
        config,
    )
...
def train(config: Config):
    ...
    only_decoder = train_only_decoder(config=config.only_decoder, checkpoint_dir=checkpoint_dir)
