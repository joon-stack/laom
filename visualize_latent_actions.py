...
def visualize(args):
    ...
    lapo = LAOMWithLabels(**model_kwargs).to(DEVICE)
    ...
    for traj_idx, t, view_key in tqdm(sampled_combinations, desc="Extracting latent actions"):
        ...
        with torch.autocast(DEVICE, dtype=torch.bfloat16):
            _, latent_action, _ = lapo(obs, future_obs)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Visualize latent actions from a LAOM model using UMAP, with color for action bin and shape for view.")
